const eventUtilsVersion = "Monday, 2026-09-07 @ 08:19:19";
    ...
    eventsByDate.forEach( ( day ) => {
        ...
        const eventElements = day.events.map( ( event ) => {
            ...
            const start_time_string =
                event.event_date +
                " " +
                ( isTimeString( event.start_time ) ? event.start_time : "12:01:00 am" );
            const start = toUTCFormat( start_time_string );
            const end_time_string =
                event.event_date +
                " " +
                ( isTimeString( event.end_time ) ? event.end_time : "11:59:00 pm" );
