...
(function () {
    ...
    var cleanSender = function (str) {
        return (str === null || str === void 0 ? void 0 : str.replace(/^([\w\s]+)\s*from\s*\1$/, '$1').replace(/lstadtlander1/, 'Linda Stadtlander').replace(/MAUREEN PETRELLA/, 'Maureen Petrella').replace(/dougallennh@gmail\.com/, 'Doug Allen').replace(/colette\.ashton50@gmail\.com/, 'Colette Ashton').replace(/dlt_nc@yahoo.com/, 'Dennis Thompson').replace(/ASTRID EWING/, 'Jim Ewing').replace(/from Letters from an American$/, '').replace(/ from Kal Draws The Line/, '').replace(/THE deEP STATE.*/, 'THE deEP STATE').replace(/Marcia Crabtree- Volunteer Manager.*/, 'Marcia Crabtree').replace(/cmhvx@aol.com/, "Chris Heimburg"));
    };
